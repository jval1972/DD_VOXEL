...
type
  ...
  public
    ...
    procedure AddgrAb(const l, t: integer);
    {$IFDEF UseDelphi}
...
{TChunkgrAb implementation}

{Assigns from another TChunkgrAb chunk}
procedure TChunkgrAb.Assign(Source: TChunk);
begin
  fLeftOffset := TChunkgrAb(Source).fLeftOffset;
  fTopOffset := TChunkgrAb(Source).fTopOffset;
end;

{Loading the chunk from a stream}
function TChunkgrAb.LoadFromStream(Stream: TStream;
  const ChunkName: TChunkName; Size: Integer): Boolean;
var
  PP: PIntegerArray;
begin
  ...
  {Get offsets}
  PP := data;
  fLeftOffset := ByteSwap(PP[0]);
  fTopOffset := ByteSwap(PP[1]);
end;

{Saving the chunk to a stream}
function TChunkgrAb.SaveToStream(Stream: TStream): Boolean;
var
  PP: PIntegerArray;
begin
  {Size is length from keyword, plus a null character to divide}
  {plus the length of the text}
  ResizeData(2 * SizeOf(Integer));
  PP := data;
  PP[0] := ByteSwap(fLeftOffset);
  PP[1] := ByteSwap(fTopOffset);
  {Let ancestor calculate crc and save}
  Result := inherited SaveToStream(Stream);
end;
...
begin
  ...
  repeat
    ...
    if ChunkName = 'grAb' then
      ...
      begin
        pp := TChunk(Chunks.Item[ChunkCount - 1]).Data;
        fLeftOffset := ByteSwap(pp^);
        inc(pp);
        fTopOffset := ByteSwap(pp^);
      end;
...
procedure TPngObject.AddgrAb(const l, t: integer);
var
  grAbChunk: TChunkgrAb;
begin
  grAbChunk := Chunks.Add(TChunkgrAb) as TChunkgrAb;
  grAbChunk.LeftOffset := l;
  grAbChunk.TopOffset := t;
end;
