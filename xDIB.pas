...
function MakeDIBPixelFormat555: TDIBPixelFormat;
function MakeDIBPixelFormat888: TDIBPixelFormat;
...
function MakeDIBPixelFormat555: TDIBPixelFormat;
begin
  Result.RBitMask := ((1 shl 5) - 1) shl 10;
  Result.GBitMask := ((1 shl 5) - 1) shl 5;
  Result.BBitMask := (1 shl 5) - 1;
  Result.RBitCount := 5;
  Result.GBitCount := 5;
  Result.BBitCount := 5;
  Result.RBitCount2 := 3;
...
function MakeDIBPixelFormat888: TDIBPixelFormat;
begin
  Result.RBitMask := ((1 shl 8) - 1) shl 16;
  Result.GBitMask := ((1 shl 8) - 1) shl 8;
  Result.BBitMask := (1 shl 8) - 1;
  Result.RBitCount := 8;
  Result.GBitCount := 8;
  Result.BBitCount := 8;
  Result.RBitCount2 := 0;
...
{$INCLUDE dib_PaletteEntriesToRGBQuads_fastloop.inc}
{$INCLUDE dib_PaletteEntriesToRGBQuads_fastloop.inc}
{$INCLUDE dib_PaletteEntriesToRGBQuads_fastloop.inc}
{$INCLUDE dib_PaletteEntriesToRGBQuads_fastloop.inc}
{$INCLUDE dib_PaletteEntriesToRGBQuads_fastloop.inc}
...
{$INCLUDE dib_PaletteEntriesToRGBQuads_fastloop.inc}
{$INCLUDE dib_PaletteEntriesToRGBQuads_fastloop.inc}
{$INCLUDE dib_PaletteEntriesToRGBQuads_fastloop_end.inc}
end;
...
{$INCLUDE dib_RGBQuadsToPaletteEntries_fastloop.inc}
{$INCLUDE dib_RGBQuadsToPaletteEntries_fastloop.inc}
{$INCLUDE dib_RGBQuadsToPaletteEntries_fastloop.inc}
{$INCLUDE dib_RGBQuadsToPaletteEntries_fastloop.inc}
{$INCLUDE dib_RGBQuadsToPaletteEntries_fastloop.inc}
...
{$INCLUDE dib_RGBQuadsToPaletteEntries_fastloop.inc}
{$INCLUDE dib_RGBQuadsToPaletteEntries_fastloop.inc}
{$INCLUDE dib_RGBQuadsToPaletteEntries_fastloop_end.inc}
end;
...
begin
  ...
  FPixelFormat := MakeDIBPixelFormat888;
end;
...
begin
  ...
  begin
    if BI.biBitCount=16 then
      APixelFormat := MakeDIBPixelFormat555
    else if BI.biBitCount=32 then
      APixelFormat := MakeDIBPixelFormat888
    else
      APixelFormat := MakeDIBPixelFormat888;
  end;
      ...
      SizeOf(Windows.TBitmap):
          begin
            ...
            case BitmapRec^.bmBitsPixel of
              16: PixelFormat := MakeDIBPixelFormat555;
            else
              PixelFormat := MakeDIBPixelFormat888;
            end;
