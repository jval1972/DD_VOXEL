...
const
  ...
  HereticPaletteRaw: array[0..767] of Byte = (
  ...
  HexenPaletteRaw: array[0..767] of Byte = (
  ...
  StrifePaletteRaw: array[0..767] of Byte = (
...
type
  TPaletteArray = array[0..255] of LongWord;
  PPaletteArray = ^TPaletteArray;

function VXE_FindAproxColorIndex(const pal: PPaletteArray; const c: LongWord;
  const start: integer = 0; const finish: integer = 255): integer;

function VXE_GetRowPalette(const gamepalette: string): pointer;

procedure VXE_RawPalette2PaletteArray(const rowpal: pointer; var palette: TPaletteArray);
...
var
  ...
  idx: integer;
begin
  if UpperCase(gamepalette) = 'DOOM' then
    result := @DoomPaletteRaw
  else if UpperCase(gamepalette) = 'HERETIC' then
    result := @HereticPaletteRaw
  else if UpperCase(gamepalette) = 'HEXEN' then
    result := @HexenPaletteRaw
  else if UpperCase(gamepalette) = 'STRIFE' then
    result := @StrifePaletteRaw
  else if UpperCase(gamepalette) = 'RADIX' then
  ...
  begin
    ...
    if FileExists(gamepalette) then
    begin
      fs := TFileStream.Create(gamepalette, fmOpenRead);
      if fs.Size = 768 then
        fs.Read(diskpal, 768)
      else if fs.Size = SizeOf(TPaletteArray) then
      begin
        GetMem(pa, SizeOf(TPaletteArray));
        fs.Read(pa^, SizeOf(TPaletteArray));
        idx := 0;
        for i := 0 to 255 do
        begin
          diskpal[idx] := GetRValue(pa[i]);
          inc(idx);
          diskpal[idx] := GetGValue(pa[i]);
          inc(idx);
          diskpal[idx] := GetBValue(pa[i]);
          inc(idx);
        end;
        FreeMem(pa, SizeOf(TPaletteArray));
      end;
      fs.Free;
    end;
