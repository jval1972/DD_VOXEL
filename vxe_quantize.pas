...
function vxe_getquantizevoxelpalette(const voxelbuffer: voxelbuffer_p; const voxelsize: Integer;
  var pal: TPaletteArray; const numcolors: integer = 256): boolean;

procedure vxe_quantizevoxeldata(const voxelbuffer: voxelbuffer_p; const voxelsize: Integer;
  const numcolors: integer = 256);

procedure vxe_quantizepalette(const lpal: PPaletteArray; const numcolors: integer = 255);
...
begin
  ...
  color_quantize_image(imgdata, imgsize, numcolors, False);
  ...
  for x := 0 to voxelsize - 1 do
    for y := 0 to voxelsize - 1 do
      for z := 0 to voxelsize - 1 do
        if voxelbuffer[x, y, z] <> 0 then
        begin
          voxelbuffer[x, y, z] := imgdata[imgsize];
          inc(imgsize);
        end;
...
begin
  ...
  for x := 0 to imgsize - 1 do
    lpal[255 - x] := imgdata[imgsize - 1 - x];

  for x := imgsize to 255 do
    lpal[255 - x] := 0;
