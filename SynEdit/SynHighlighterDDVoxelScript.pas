...
unit SynHighlighterDDVoxelScript;
{$ENDIF}
...
type
  TSynDDVoxelScriptHighlighter = class(TSynCustomHighlighter)
  private
...
function TSynDDVoxelScriptHighlighter.IsIdentChar(AChar: WideChar): Boolean;
var
  i: Integer;
begin
  Result := False;
...
function TSynDDVoxelScriptHighlighter.IsKeyword(const AKeyword: UnicodeString): Boolean;
var
...
function TSynDDVoxelScriptHighlighter.IsWordBreakChar(AChar: WideChar): Boolean;
begin
  Result := inherited IsWordBreakChar(AChar) and not IsIdentChar(AChar);
end;

constructor TSynDDVoxelScriptHighlighter.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  fKeyWords := TUnicodeStringList.Create;
  // VoxelScript Keywords
  fKeyWords.Add('BEGIN');
  fKeyWords.Add('END');
  fKeyWords.Add('GOTO');
...
destructor TSynDDVoxelScriptHighlighter.Destroy;
begin
  fKeyWords.Free;
  inherited Destroy;
end; { Destroy }

procedure TSynDDVoxelScriptHighlighter.AnsiProc;
begin
  case fLine[Run] of
     #0: NullProc;
    #10: LFProc;
...
procedure TSynDDVoxelScriptHighlighter.PasStyleProc;
begin
  case fLine[Run] of
     #0: NullProc;
    #10: LFProc;
...
procedure TSynDDVoxelScriptHighlighter.CStyleProc;
begin
  case fLine[Run] of
     #0: NullProc;
    #10: LFProc;
...
procedure TSynDDVoxelScriptHighlighter.AsciiCharProc;
begin
...
procedure TSynDDVoxelScriptHighlighter.BraceOpenProc;
begin
  fTokenID := tkComment;
  fRange := rsPasStyle;
  inc(Run);
...
procedure TSynDDVoxelScriptHighlighter.PointCommaProc;
begin
  inc(Run);
  fTokenID := tkSymbol;
end;

procedure TSynDDVoxelScriptHighlighter.CRProc;
begin
  fTokenID := tkSpace;
  Inc(Run);
...
procedure TSynDDVoxelScriptHighlighter.IdentProc;
begin
...
procedure TSynDDVoxelScriptHighlighter.IntegerProc;
...
procedure TSynDDVoxelScriptHighlighter.LFProc;
begin
  fTokenID := tkSpace;
  inc(Run);
end;

procedure TSynDDVoxelScriptHighlighter.NullProc;
begin
  fTokenID := tkNull;
  inc(Run);
end;

procedure TSynDDVoxelScriptHighlighter.NumberProc;
...
procedure TSynDDVoxelScriptHighlighter.RoundOpenProc;
begin
  inc(Run);
  fTokenId := tkSymbol;
end;

procedure TSynDDVoxelScriptHighlighter.SlashProc;
begin
  Inc(Run);
  if FLine[Run] = '/' then
  begin
...
procedure TSynDDVoxelScriptHighlighter.SpaceProc;
begin
  inc(Run);
  fTokenID := tkSpace;
...
procedure TSynDDVoxelScriptHighlighter.StringProc;
var
   delim : WideChar;
begin
  fTokenID := tkString;
...
procedure TSynDDVoxelScriptHighlighter.UnknownProc;
begin
  inc(Run);
  fTokenID := tkUnknown;
end;

procedure TSynDDVoxelScriptHighlighter.Next;
begin
  fTokenPos := Run;
  case fRange of
    rsAnsi: AnsiProc;
...
function TSynDDVoxelScriptHighlighter.GetDefaultAttribute(Index: integer): TSynHighlighterAttributes;
begin
  case Index of
...
function TSynDDVoxelScriptHighlighter.GetEol: Boolean;
begin
  Result := Run = fLineLen + 1;
end;

function TSynDDVoxelScriptHighlighter.GetRange: Pointer;
begin
  Result := Pointer(fRange);
end;

function TSynDDVoxelScriptHighlighter.GetTokenID: TtkTokenKind;
begin
  Result := fTokenId;
end;
...
// GetCharBeforeToken
//
function TSynDDVoxelScriptHighlighter.GetCharBeforeToken(offset : Integer = -1) : WideChar;
begin
...
// GetCharAfterToken
//
function TSynDDVoxelScriptHighlighter.GetCharAfterToken(offset : Integer = 1) : WideChar;
begin
   Result := FLine[fTokenPos + offset];
end;

function TSynDDVoxelScriptHighlighter.GetTokenAttribute: TSynHighlighterAttributes;
begin
  case fTokenID of
...
function TSynDDVoxelScriptHighlighter.GetTokenKind: integer;
begin
  Result := Ord(fTokenId);
end;

procedure TSynDDVoxelScriptHighlighter.ResetRange;
begin
  fRange := rsUnknown;
end;

procedure TSynDDVoxelScriptHighlighter.SetRange(Value: Pointer);
begin
  fRange := TRangeState(Value);
end;

procedure TSynDDVoxelScriptHighlighter.SetKeyWords(const Value: TUnicodeStrings);
var
  i: Integer;
begin
  if Value <> nil then
...
class function TSynDDVoxelScriptHighlighter.GetLanguageName: string;
begin
  Result := 'DDVoxelScript';
end;

{$IFNDEF SYN_CLX}
function TSynDDVoxelScriptHighlighter.LoadFromRegistry(RootKey: HKEY; Key: string): boolean;
var
  r: TBetterRegistry;
begin
...
function TSynDDVoxelScriptHighlighter.SaveToRegistry(RootKey: HKEY; Key: string): boolean;
var
  r: TBetterRegistry;
begin
  r := TBetterRegistry.Create;
...
function TSynDDVoxelScriptHighlighter.GetStringDelim: TStringDelim;
begin
  Result := fStringDelim;
end;

procedure TSynDDVoxelScriptHighlighter.SetStringDelim(const Value: TStringDelim);
begin
   fStringDelim := Value;
end;

function TSynDDVoxelScriptHighlighter.GetIdentifierChars: UnicodeString;
begin
  Result := fIdentChars;
end;

procedure TSynDDVoxelScriptHighlighter.SetIdentifierChars(const Value: UnicodeString);
begin
  fIdentChars := Value;
end;

function TSynDDVoxelScriptHighlighter.StoreIdentChars: Boolean;
begin
   Result := (fIdentChars <> cDefaultIdentChars);
end;

procedure TSynDDVoxelScriptHighlighter.SetDetectPreprocessor(Value: boolean);
begin
...
class function TSynDDVoxelScriptHighlighter.GetFriendlyLanguageName: UnicodeString;
begin
  Result := 'DelphiDoom VoxelScript';
end;

// IsStringDelim
//
function TSynDDVoxelScriptHighlighter.IsStringDelim(aChar : WideChar) : Boolean;
begin
...
initialization
{$IFNDEF SYN_CPPB_1}
  RegisterPlaceableHighlighter(TSynDDVoxelScriptHighlighter);
{$ENDIF}
end.
